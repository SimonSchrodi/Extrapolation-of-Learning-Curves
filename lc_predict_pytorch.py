...
def train_model(model, loss, optimizer, train_X, train_Y, b_size, epochs):
    ...
    for n_epoch in range(epochs):
        ...
        for b in range(n_full_batches):
            outputs = model(train_X[start_index: start_index + b_size].unsqueeze_(1))
...
if __name__ == "__main__":
    ...
    model = LCPredict()  # our data would be float, so model also needs to be float
    ...
    # train_model(model, loss_fn, optimizer_adam, train_X, train_Y, 16, 100)
