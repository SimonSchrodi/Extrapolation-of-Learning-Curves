...
def get_data(train_data, val_data, test_data, train_targets, val_targets, test_targets):
    ...
    train_data_new_X = np.zeros((1000, 17, 1))
    train_data_new_Y = np.zeros((1000, 1))

    val_data_X = np.zeros((500, 17, 1))
    val_data_Y = np.zeros((500, 1))

    test_data_X = np.zeros((500, 17, 1))
    ...
    for index, data in enumerate(train_data):
        ...
        # ------- dynamic params -----------#
        train_data_new_X[index, 7] = data["Train/val_accuracy"][0]
        train_data_new_X[index, 8] = data["Train/val_accuracy"][1]
        train_data_new_X[index, 9] = data["Train/val_accuracy"][2]
        train_data_new_X[index, 10] = data["Train/val_accuracy"][3]
        train_data_new_X[index, 11] = data["Train/val_accuracy"][4]
        train_data_new_X[index, 12] = data["Train/val_accuracy"][5]
        train_data_new_X[index, 13] = data["Train/val_accuracy"][6]
        train_data_new_X[index, 14] = data["Train/val_accuracy"][7]
        train_data_new_X[index, 15] = data["Train/val_accuracy"][8]
        train_data_new_X[index, 16] = data["Train/val_accuracy"][9]
    ...
    for index, data in enumerate(val_data):
        ...
        # ------- dynamic params -----------#
        val_data_X[index, 7] = data["Train/val_accuracy"][0]
        val_data_X[index, 8] = data["Train/val_accuracy"][1]
        val_data_X[index, 9] = data["Train/val_accuracy"][2]
        val_data_X[index, 10] = data["Train/val_accuracy"][3]
        val_data_X[index, 11] = data["Train/val_accuracy"][4]
        val_data_X[index, 12] = data["Train/val_accuracy"][5]
        val_data_X[index, 13] = data["Train/val_accuracy"][6]
        val_data_X[index, 14] = data["Train/val_accuracy"][7]
        val_data_X[index, 15] = data["Train/val_accuracy"][8]
        val_data_X[index, 16] = data["Train/val_accuracy"][9]
    ...
    for index, data in enumerate(test_data):
        ...
        # ------- dynamic params -----------#
        test_data_X[index, 7] = data["Train/val_accuracy"][0]
        test_data_X[index, 8] = data["Train/val_accuracy"][1]
        test_data_X[index, 9] = data["Train/val_accuracy"][2]
        test_data_X[index, 10] = data["Train/val_accuracy"][3]
        test_data_X[index, 11] = data["Train/val_accuracy"][4]
        test_data_X[index, 12] = data["Train/val_accuracy"][5]
        test_data_X[index, 13] = data["Train/val_accuracy"][6]
        test_data_X[index, 14] = data["Train/val_accuracy"][7]
        test_data_X[index, 15] = data["Train/val_accuracy"][8]
        test_data_X[index, 16] = data["Train/val_accuracy"][9]
...
def test_model(model, test_X, test_Y):
    ...
    return predictions, error_abs, error_signed, error_mean, error_max, error_min



def compute_mse_rmse_error(predictions, trueVal):
    """
    Function computes the mean squared and root mean squared error
    mse = (pred - true)^2
    :param predictions: predicted values
    :param true: true values
    :return: tuple, rmse and mse
    """
...
if __name__ == "__main__":
    ...
    # data_path = '/content/drive/My Drive/ColabWorks/DL_project/DATA/fashion_mnist.json' # use this in colab
    ...
    lr_list = [0.0008, 0.0008, 0.0008, 0.0008, 0.0008]
    ...
    for i in range(5):
        ...
        # annotate - write data points every 10 points
        for k in range(0, len(loss_list_epoch), 20):
            label = "{:.4f}".format(loss_list_epoch[k])
            plt.annotate(label, xy=(x_val[k], loss_list_epoch[k]))
        ...
        plt.savefig("loss_model_"+str(i)+'.png')
        plt.close()

        predictions, error_abs, error_signed, error_mean, error_max, error_min = \
            test_model(model, test_X_tensor, test_Y_tensor)

        error_mean = error_mean * 100  # to cancel the normalization
        mean_error_list.append(error_mean)  # to be saved later

        x_val = np.arange(0, 500)

        # save absolute and signed errors as npy files, has to be scaled up by 100 to counter for normalization,
        # these are percentages
        error_abs = np.array(error_abs) * 100
        error_signed = np.array(error_signed) * 100
        np.save("error_abs_model_"+str(i), error_abs)
        np.save("error_signed_model_"+str(i), error_signed)

        #----------------- plotting and annotating -------------------#
        # plot and save error abs and error signed
        plt.plot(x_val, error_abs)
        plt.xlabel("Test samples")
        plt.ylabel("Error abs")
        plt.grid(color='r', linewidth=1)

        # annotate every 10 sample value
        for k in range(0, 500, 10):
            label = "{:.2f}".format(error_abs[k])
            plt.annotate(label, xy=(x_val[k], error_abs[k]))

        plt.show(block=False)
        plt.pause(3)
        # plt.close()
        plt.savefig("error_abs_model_" + str(i) + '.pdf')
        plt.savefig("error_abs_model_" + str(i) + '.png')
        plt.close()

        plt.plot(x_val, error_signed)
        plt.xlabel("Test samples")
        plt.ylabel("Error signed")
        plt.grid(color='r', linewidth=1)

        # annotate every 10 sample value
        for k in range(0, 500, 10):
            label = "{:.2f}".format(error_signed[k])
            plt.annotate(label, xy=(x_val[k], error_signed[k]))

        plt.show(block=False)
        plt.pause(3)
        # plt.close()
        plt.savefig("error_signed_model_" + str(i) + '.pdf')
        plt.savefig("error_signed_model_" + str(i) + '.png')
        plt.close()


        # get mse, rmse and save
        mse, rmse = compute_mse_rmse_error(predictions, test_Y_tensor)
        mse_list.append(mse.item())
        rmse_list.append(rmse.item())
        max_error_list.append(error_max*100)

        # write to file
        fd.write("\nerror_abs : "+str(error_abs))
        fd.write("\nerror_signed : "+str(error_signed))
        fd.write("\nerror_mean : "+str(error_mean))
        fd.write("\nerror_mse : "+str(mse))
        fd.write("\nerror_rmse : "+str(rmse))
        fd.write("\nerror_max : "+str(error_max*100))
        fd.write("\nerror_min : "+str(error_min*100))

        avg_max_error += (error_max * 100)  # sum up all max erros and compute average for 5 models
        avg_mean_error += (error_mean * 100)
        avg_mse_error += mse
        avg_rmse_error += rmse

    # compute the average across all 5 models
    avg_max_error /= 5
    avg_mean_error /= 5
    avg_mse_error /= 5
    avg_rmse_error /= 5
    ...
    print("-------- Ensemble ---------------")
    print("avg_max_error_5_models : ", avg_max_error)
    print("avg_mean_error_5_models : ", avg_mean_error)
    print("avg_mse_error_5_models : ", avg_mse_error)
    print("avg_rmse_error_5_models : ", avg_rmse_error)

    print("------------- Individual model - best results -------------")
    print("Max error : ", min(max_error_list))
    print("MSE error : ", min(mse_list))
    print("RMSE error : ", min(rmse_list))

    print("max list : ", max_error_list)
    ...
    # save the mse and rmse for all models
    np.save("mse_5_models", np.array(mse_list))
    np.save("rmse_5_models", np.array(rmse_list))
    np.save("mean_error_5_models", np.array(mean_error_list))

    x_val = np.arange(0, len(mse_list))
    plt.plot(x_val, mse_list)
    plt.grid(color='r', linewidth=1)
    plt.xlabel("model number")
    plt.ylabel("mse")

    # annotate every sample value
    for i in range(0, 5):
        label = "{:.2f}".format(mse_list[i])
        plt.annotate(label, xy=(x_val[i], mse_list[i]))

    plt.show(block=False)
    plt.pause(3)
    plt.savefig("mse_error_5_models.pdf")
    plt.savefig("mse_error_5_models.png")
    plt.close()

    #----- plot rmse 5 models ------------#
    x_val = np.arange(0, len(rmse_list))
    plt.plot(x_val, rmse_list)
    plt.grid(color='r', linewidth=1)
    plt.xlabel("model number")
    plt.ylabel("rmse")

    for i in range(0, 5):
        label = "{:.2f}".format(rmse_list[i])
        plt.annotate(label, xy=(x_val[i], rmse_list[i]))

    plt.show(block=False)
    plt.pause(3)
    plt.savefig("rmse_error_5_models.pdf")
    plt.savefig("rmse_error_5_models.png")
    plt.close()

    #----- plot mean error 5 models --------#
    x_val = np.arange(0, len(mean_error_list))
    plt.plot(x_val, mean_error_list)
    plt.grid(color='r', linewidth=1)
    plt.xlabel("model number")
    plt.ylabel("mean error")

    for i in range(0, 5):
        label = "{:.2f}".format(mean_error_list[i])
        plt.annotate(label, xy=(x_val[i], mean_error_list[i]))

    plt.show(block=False)
    plt.pause(3)
    plt.savefig("mean_error_5_models.pdf")
    plt.savefig("mean_error_5_models.png")
    plt.close()
